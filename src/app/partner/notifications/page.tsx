...
type Notification = {
  id: string; type: string; title_ar: string; body_ar: string
...
const typeIcon: Record<string, string> = {
  ...
  subscription_confirmed: '🏅', team_approved: '🏆',
  activation_approved: '✅', activation_rejected: '❌',
  general: '🔔',
}

const entityRoute: Record<string, string> = {
  booking: '/partner/bookings',
  tournament_team: '/partner/tournaments',
  subscription: '/partner/academies',
}
      ...
      <div style={{ padding: '16px', display: 'flex', flexDirection: 'column', gap: 10 }}>
        {loading ? (
          ...
          <div style={{ textAlign: 'center', padding: '60px 0' }}>
            ...
            <p style={{ fontSize: 13, color: 'var(--text3)', marginTop: 4 }}>ستظهر هنا إشعارات الحجوزات والتسجيلات</p>
          </div>
        ) : notes.map((n) => (
          <button
            key={n.id}
            onClick={() => n.entity_type && entityRoute[n.entity_type] && router.push(entityRoute[n.entity_type])}
            style={{
          ...
          >
            ...
            {!n.is_read && <span style={{ width: 8, height: 8, borderRadius: 4, background: 'var(--primary)', flexShrink: 0, marginTop: 6 }} />}
          </button>
        ))}
      </div>
    </div>
